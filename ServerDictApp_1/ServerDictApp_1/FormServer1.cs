...
namespace ServerDictApp_1
{
    public partial class FormServer1 : Form
    {
        public FormServer1()
        {
            ...
            btn_shutdown.Click += new EventHandler(btn_shutdown_Click);
        ...
        private Data GetResponse(string serialized)
        {
            ...
            else if (deserialized.code == 400)
            {
                if (Global.Capacity >= Global.MaxCapacity)
                    return new Data(code: 404, content: "", dest: deserialized.src, src: deserialized.dest);
                Global.Capacity++;
                return new Data(code: 402, content: "", dest: deserialized.src, src: deserialized.dest);
            }
            else if (deserialized.code == 500)
            {
                Global.Capacity--;
                return new Data(code: 502, content: "", dest: deserialized.src, src: deserialized.dest);
            }
